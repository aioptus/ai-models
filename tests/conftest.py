...
def pytest_sessionfinish(session, exitstatus):
    if importlib.util.find_spec("aioptuspackage"):
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "uninstall", "-y", "aioptuspackage"]
            )
            shutil.rmtree("tests/test_package/build")
            shutil.rmtree("tests/test_package/aioptuspackage.egg-info")
        except subprocess.CalledProcessError as ex:
            warnings.warn(f"Error occurred uninstalling aioptuspackage: {ex}")
