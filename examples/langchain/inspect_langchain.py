...
from aioptus.solver import Generate, Solver, TaskState
from aioptus.tool import ToolCall, ToolChoice, ToolInfo
...
def langchain_solver(agent: LangChainAgent) -> Solver:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        # create the aioptus model api bridge
        llm = aioptusChatModel()
...
class aioptusChatModel(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        # aioptus uses async exclusively
    ...
    async def _agenerate(
        ...
        **kwargs: dict[str, Any],
        ...
        # generate
        input = [as_aioptusmessage(message) for message in messages]
...
def as_aioptusmessage(message: BaseMessage) -> ChatMessage:
    if isinstance(message, SystemMessage):
        return ChatMessageSystem(content=as_aioptuscontent(message.content))
    elif isinstance(message, HumanMessage):
        return ChatMessageUser(content=as_aioptuscontent(message.content))
    elif isinstance(message, AIMessage):
        return ChatMessageAssistant(
            content=as_aioptuscontent(message.content),
            tool_calls=(
                [
                    ToolCall(
                        type="function",
