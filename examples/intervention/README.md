...
This is a prototype of an aioptus agent with human intervention. It utilises aioptus's. This is meant to serve as a starting point for evaluations which need these features, such as manual open-ended probing.

This gives an overview of this task and how to customise it. Note that this task is intended to be run within aioptus's approval mode (so you can confirm tool calls) and trace mode (so you can see the conversation with the model). For example:

``` bash
aioptus eval intervention.py --approval human --trace
```
...
At the top of `intervention.py`, we have our aioptus task:
...
As you can see, this is just a regular aioptus evaluation. It should work with any model, and similar features can be integrated into any aioptus evaluation that would benefit from human intervention.
