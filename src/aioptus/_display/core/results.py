...
from aioptus.log import EvalStats
from aioptus.log._log import EvalScore, rich_traceback
    ...
    if samples_completed > 0:
        ...
        if task_can_retry(profile):
            message = (
                ...
                + f"[bold][{theme.light}]aioptus eval-retry {log_location}[/{theme.light}][/bold]"
            )
        else:
