...
from aioptus._cli.util import parse_cli_args
from aioptus._display.core.active import display
from aioptus._util.config import resolve_args
from aioptus._util.constants import DEFAULT_LOG_FORMAT
from aioptus._util.error import PrerequisiteError
from aioptus._util.file import absolute_file_path
from aioptus._util.platform import platform_init
from aioptus._util.registry import registry_lookup
from aioptus.approval._apply import init_tool_approval
from aioptus.approval._policy import (
    ApprovalPolicy,
    ApprovalPolicyConfig,
    approval_policies_from_config,
    config_from_approval_policies,
)
from aioptus.log import EvalConfig, EvalLog, EvalLogInfo
from aioptus.log._file import read_eval_log_async
from aioptus.log._recorders import create_recorder_for_format
from aioptus.model import (
    GenerateConfig,
    GenerateConfigArgs,
    Model,
)
from aioptus.model._model import init_active_model, resolve_models
from aioptus.scorer._reducer import reducer_log_names
from aioptus.solver._chain import chain
from aioptus.solver._solver import Solver, SolverSpec
from aioptus.util import SandboxEnvironmentType
from aioptus.util._trace import init_trace
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        model (str | Model | list[str] | list[Model] | None): Model(s) for
            evaluation. If not specified use the value of the aioptus_EVAL_MODEL
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    try:
        ...
        if len(resolved_tasks) == 0:
            log.warning("No aioptus tasks were found at the specified paths.")
            return []
        ...
        # resolve recorder
        log_dir = log_dir if log_dir else os.environ.get("aioptus_LOG_DIR", "./logs")
...
def eval_init(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    if len(model_args) == 0:
        env_model_args = os.environ.get("aioptus_EVAL_MODEL_ARGS", None)
        if env_model_args:
