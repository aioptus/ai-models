...
from aioptus._util.logger import init_logger
from aioptus._view.server import view_server
...
def view(
    ...
    log_dir = log_dir if log_dir else os.getenv("aioptus_LOG_DIR", "./logs")
...
def view_acquire_port(port: int) -> None:
    ...
    if pid_file.exists():
        ...
        try:
            ...
            p.terminate()
            display().print(
                f"Terminating existing aioptus view command using port {port}"
            )
            p.wait(WAIT_SECONDS)
