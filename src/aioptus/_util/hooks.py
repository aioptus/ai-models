...
# environment variable (e.g. aioptus_TELEMETRY='mypackage.send_telemetry')
...
# Telemetry (aioptus_TELEMETRY)
#
# Telemetry can be optionally enabled by setting an aioptus_TELEMETRY
...
# API Key Override (aioptus_API_KEY_OVERRIDE)
#
# API Key overrides can be optionally enabled by setting an
# aioptus_API_KEY_OVERRIDE environment variable which conforms to the
...
def init_hooks() -> None:
    ...
    if len(messages) > 0:
        ...
        print(
            f"[blue][bold]aioptus v{version}[/bold][/blue]\n[bright_black]{all_messages}[/bright_black]\n"
        )
