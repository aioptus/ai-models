...
def _import_object(
    ...
    if msg is None:
        if aioptus.isclass(_object):
            _type = "class"
        elif aioptus.isfunction(_object):
            _type = "function"
        else:
            _type = "attribute"
        msg = (
...
def deprecation_warning(
    ...
    if calling_frame is not None:
        info = aioptus.getframeinfo(calling_frame)
...
def default_deprecation_msg(
    ...
    """
    if user_msg is None:
        if aioptus.isclass(obj):
            _obj = " class"
        elif aioptus.ismethod(obj):
            _obj = " method"
        elif aioptus.isfunction(obj) or aioptus.isbuiltin(obj):
            _obj = " function"
        else:
