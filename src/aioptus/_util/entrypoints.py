...
def ensure_entry_points(package: str | None = None) -> None:
    ...
    global _aioptusai_eps_loaded_all
    if _aioptusai_eps_loaded_all:
        return None

    # have we already loaded entry points for this package?
    if package in _aioptusai_eps_loaded:
        return None

    # enumerate entry points
    eps = entry_points(group="aioptus")
    for ep in eps:
        try:
...
# aioptus extension entry points
_aioptusai_eps_loaded: list[str] = []
_aioptusai_eps_loaded_all: bool = False
