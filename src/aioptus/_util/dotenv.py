...
aioptus_LOG_DIR_VAR = "aioptus_LOG_DIR"
...
def init_dotenv() -> None:
    ...
    if dotenv_file:
        # is there an aioptus_LOG_DIR currently in the environment? (we will give it preference)
        environment_log_dir = os.environ.get(aioptus_LOG_DIR_VAR, None)
        dotenv_log_dir = dotenv_values(dotenv_file).get(aioptus_LOG_DIR_VAR, None)
        ...
        if environment_log_dir == dotenv_log_dir or not environment_log_dir:
            ...
            aioptuslog_dir = absolute_file_path(environment_log_dir)
        ...
        if aioptuslog_dir:
            os.environ[aioptus_LOG_DIR_VAR] = aioptuslog_dir
