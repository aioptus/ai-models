...
from aioptus._util.constants import SANDBOX
...
def task_project_name(task: str) -> str:
    ...
    return f"aioptus-{task}-i{uuid().lower()}"


aioptusproject_pattern = r"^aioptus-[a-z\d\-_]*-i[a-z\d]{22}$"


def is_aioptusproject(name: str) -> bool:
    return re.match(aioptusproject_pattern, name) is not None
