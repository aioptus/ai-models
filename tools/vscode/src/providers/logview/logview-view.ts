...
const kLogViewId = "aioptus.logview";


export class aioptusViewManager {
  constructor(
    private readonly context_: ExtensionContext,
    private readonly webViewManager_: aioptusViewWebviewManager,
    private readonly envMgr_: WorkspaceEnvManager,
    logsWatcher: aioptusLogsWatcher
  ) {

    this.context_.subscriptions.push(logsWatcher.onaioptusLogCreated(async (e) => {
...
export class aioptusViewWebviewManager extends aioptusWebviewManager<
  aioptusViewWebview,
  LogviewState
> {
  constructor(
    ...
    context.subscriptions.push(
      aioptusManager.onaioptusChanged((e: aioptusChangedEvent) => {
  ...
  protected override getWorkspaceState(): LogviewState | undefined {
    const data: Record<string, string> = this.context_.workspaceState.get(this.kaioptusViewState, {});
    if (data) {
      return {
  ...
  protected setWorkspaceState(state: LogviewState) {
    void this.context_.workspaceState.update(this.kaioptusViewState, {
  ...
  private kaioptusViewState = 'aioptusViewState';
...
class aioptusViewWebview extends aioptusWebview<LogviewState> {
  ...
  public setManager(manager: aioptusViewWebviewManager) {
    if (this._manager !== manager) {
      this._manager = manager;
    }
  }
  _manager: aioptusViewWebviewManager | undefined;
