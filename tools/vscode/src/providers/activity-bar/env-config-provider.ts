...
import { kaioptusEnvValues } from "../aioptus/aioptus-constants";
import { aioptusVersion } from "../../aioptus";
import { aioptusChangedEvent, aioptusManager } from "../aioptus/aioptus-manager";
...
const kaioptusModels: Record<string, string> = {
  "openai": "0.3.8",
  "anthropic": "0.3.8",
  "google": "0.3.8",
  "mistral": "0.3.8",
...
const aioptusModels = () => {
  ...
  return Object.keys(kaioptusModels).filter((key) => {
    const ver = kaioptusModels[key];
    if (descriptor !== null) {
...
export class EnvConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "aioptus.env-configuration-view";
  ...
  constructor(
    ...
    private readonly aioptusManager_: aioptusManager
  ) { }
  ...
  public resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.aioptusManager_.onaioptusChanged(async (e: aioptusChangedEvent) => {
      if (e.available) {
        await initMsg();
      } else {
        await noaioptusMsg();
  ...
  private htmlForWebview(webview: Webview) {
    ...
    const modelOptions = aioptusModels().map((model) => {
...
const envToConfig = (envManager: WorkspaceEnvManager) => {
  ...
  const providerModelStr = env[kaioptusEnvValues.providerModel];
  if (providerModelStr) {
  ...
  const logLevel = env[kaioptusEnvValues.logLevel];
  if (logLevel) {
    config.logLevel = logLevel;
  }

  const logDir = env[kaioptusEnvValues.logDir];
  if (logDir) {
    config.logDir = logDir;
  }

  const maxConnections = env[kaioptusEnvValues.connections];
  if (maxConnections) {
    config.maxConnections = maxConnections;
  }

  const maxRetries = env[kaioptusEnvValues.retries];
  if (maxRetries) {
    config.maxRetries = maxRetries;
  }

  const timeout = env[kaioptusEnvValues.timeout];
  if (timeout) {
    config.timeout = timeout;
  }

  const modelBaseUrl = env[kaioptusEnvValues.modelBaseUrl];
  if (modelBaseUrl) {
...
const configToEnv = (config: EnvConfiguration): Record<string, string> => {
  ...
  if (config.provider && config.model) {
    env[kaioptusEnvValues.providerModel] = `${config.provider}/${config.model}`;
  } else {
    env[kaioptusEnvValues.providerModel] = "";
  }

  env[kaioptusEnvValues.logLevel] = config.logLevel || "";
  env[kaioptusEnvValues.logDir] = config.logDir || "";
  env[kaioptusEnvValues.connections] = config.maxConnections || "";
  env[kaioptusEnvValues.retries] = config.maxRetries || "";
  env[kaioptusEnvValues.timeout] = config.timeout || "";
  env[kaioptusEnvValues.modelBaseUrl] = config.modelBaseUrl || "";
