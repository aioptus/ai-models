...
export type NoaioptusCmd = {
  command: "noaioptus";
};

export class TaskConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "aioptus.task-configuration";
  ...
  public async resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.aioptusManager_.onaioptusChanged(async () => {
...
const updateSidebarState = async (taskInfo?: DocumentTaskInfo) => {
  await commands.executeCommand(
    "setContext",
    "aioptus.task-configuration.task-active",
    taskInfo !== undefined
  );
};
