...
export class ShowTaskList implements Command {
  ...
  private static readonly id = "aioptus.taskOutlineList";
  public readonly id = ShowTaskList.id;
}

export class RunSelectedEvalCommand implements Command {
  constructor(private readonly aioptusEvalMgr_: aioptusEvalManager) { }
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const evalPromise = this.aioptusEvalMgr_.startEval(
      toAbsolutePath(path),
      task,
      false
    );
    const resumeFocusCommand = "aioptus.task-outline-view.focus";
    scheduleReturnFocus(resumeFocusCommand);
    await evalPromise;
  }
  private static readonly id = "aioptus.runSelectedTask";
...
export class DebugSelectedEvalCommand implements Command {
  ...
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const task =
      treeItem.taskPath.type === "task" ? treeItem.taskPath.name : undefined;
    await this.aioptusEvalMgr_.startEval(toAbsolutePath(path), task, true);
  }
  private static readonly id = "aioptus.debugSelectedTask";
...
export class EditSelectedTaskCommand implements Command {
  constructor(
    ...
    private aioptusLogviewManager_: aioptusViewManager,
  ...
  async execute() {
    ...
    if (this.tree_.selection.length === 1) {
      ...
      const logViewColumn = this.aioptusLogviewManager_.viewColumn();
  ...
  public static readonly id = "aioptus.editSelectedTask";
...
export class CreateTaskCommand implements Command {
  ...
  private static readonly id = "aioptus.createTask";
