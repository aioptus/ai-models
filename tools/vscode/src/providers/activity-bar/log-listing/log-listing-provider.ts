...
export async function activateLogListing(
  ...
  viewServer: aioptusViewServer,
  logsWatcher: aioptusLogsWatcher
): Promise<[Command[], vscode.Disposable[]]> {

  const kLogListingDir = "aioptus.logListingDir";
  const disposables: vscode.Disposable[] = [];

  await vscode.commands.executeCommand(
    "setContext",
    "aioptus.haveEvalLogFormat",
    hasMinimumaioptusVersion(kaioptusEvalLogFormatVersion)
  );
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListing', async () => {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingReveal', async (uri?: Uri) => {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingRefresh', () => {
    treeDataProvider.refresh();
  }));

  // Register Reveal in Explorer command
  disposables.push(vscode.commands.registerCommand('aioptus.logListingRevealInExplorer', async (node: LogNode) => {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingOpenInJSONEditor', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingDeleteLogFile', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingCopyLogPath', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('aioptus.logListingCopyEditorPath', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(logsWatcher.onaioptusLogCreated((e) => {
...
export async function revealLogListing() {
  ...
  await vscode.commands.executeCommand(`workbench.view.extension.aioptus-activity-bar`);
}
