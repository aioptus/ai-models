...
export async function activateTaskOutline(
  ...
  const checkaioptus = async () => {
    const aioptusAvailable = aioptusVersion() !== null;
    await commands.executeCommand(
      "setContext",
      "aioptus.task-outline-view.noaioptus",
      !aioptusAvailable
    );
    if (aioptusAvailable) {
  ...
  context.subscriptions.push(
    aioptusManager.onaioptusChanged(async () => {
      await checkaioptus();
    })
  );
  await checkaioptus();
  ...
  return [
    [
      ...
      new RunSelectedEvalCommand(aioptusEvalMgr),
      new DebugSelectedEvalCommand(aioptusEvalMgr),
      new EditSelectedTaskCommand(tree, aioptusLogviewManager, activeTaskManager),
...
export class TaskOutLineTreeDataProvider
  ...
  public static readonly viewType = "aioptus.task-outline-view";
  constructor(
      ...
      this.workspaceMgr.onTasksChanged(
        throttle(
          async (e: TasksChangedEvent) => {
            ...
            await commands.executeCommand(
              "setContext",
              "aioptus.task-outline-view.tasksLoaded",
              true
            );
            await commands.executeCommand(
              "setContext",
              "aioptus.task-outline-view.noTasks",
              e.tasks?.length === 0
            );
          },
          500,
  ...
  private getNodes(tree: TaskPath[], parent?: TaskTreeItem): TaskTreeItem[] {
    const mode =
      workspace.getConfiguration("aioptus").get("taskListView") || "tree";
