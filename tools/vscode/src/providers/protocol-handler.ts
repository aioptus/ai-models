...
export function activateProtocolHandler(context: ExtensionContext) {
  const protocolHandler = new aioptusProtocolHandler();
  context.subscriptions.push(window.registerUriHandler(protocolHandler));
}

export class aioptusProtocolHandler implements UriHandler {
    ...
    switch (command) {
      ...
      case "open": {
        ...
        if (logFile) {
          ...
          await commands.executeCommand('aioptus.openLogViewer', logUri);
        }
      }
    }
  }
