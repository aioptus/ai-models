...
import { aioptusLastEvalPaths } from "../../aioptus/props";
import { existsSync, readFileSync, statSync } from "fs";
import { log } from "../../core/log";
import { WorkspaceStateManager } from "../workspace/workspace-state-provider";
import { withMinimumaioptusVersion } from "../../aioptus/version";
import { kaioptusChangeEvalSignalVersion } from "./aioptus-constants";
import { resolveToUri } from "../../core/uri";

export interface aioptusLogCreatedEvent {
  log: Uri
  externalWorkspace: boolean;
}

export class aioptusLogsWatcher implements Disposable {
  constructor(
    ...
    const evalSignalFiles = aioptusLastEvalPaths().map(path => path.path);
      ...
      for (const evalSignalFile of evalSignalFiles) {
        if (existsSync(evalSignalFile)) {
          ...
          if (updated > this.lastEval_) {
            ...
            withMinimumaioptusVersion(
              kaioptusChangeEvalSignalVersion,
              () => {
                // 0.3.10- or later
            ...
            if (evalLogPath !== undefined) {
              ...
              try {
                ...
                this.onaioptusLogCreated_.fire({ log: logUri, externalWorkspace });
              } catch (error) {
  ...
  private readonly onaioptusLogCreated_ =
    new EventEmitter<aioptusLogCreatedEvent>();
  public readonly onaioptusLogCreated: Event<aioptusLogCreatedEvent> =
    this.onaioptusLogCreated_.event;
