...
import { kPythonPackageName } from "../../aioptus/props";
import { removeFilesSync } from "../../core/file";


export function activateaioptusCommands(
  stateManager: WorkspaceStateManager,
  context: ExtensionContext) {
  const aioptusCommands = new aioptusCommandDispatcher(stateManager);
  context.subscriptions.push(aioptusCommands);
}


export class aioptusCommandDispatcher implements Disposable {

  constructor(stateManager: WorkspaceStateManager) {
    // init commands dir and remove any existing commands
    this.commandsDir_ = aioptusCommandsDir(stateManager);
...
function aioptusCommandsDir(stateManager: WorkspaceStateManager): string {
