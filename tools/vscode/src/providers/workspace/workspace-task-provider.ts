...
export function activateWorkspaceTaskProvider(aioptusManager: aioptusManager, context: ExtensionContext) {

  // The task manager
  const taskManager = new WorkspaceTaskManager();

  // If the interpreter changes, refresh the tasks
  context.subscriptions.push(aioptusManager.onaioptusChanged(async (e: aioptusChangedEvent) => {
    if (e.available) {
...
export class WorkspaceTaskManager {
  ...
  public async refresh() {
    ...
    try {
      const tasks = await aioptusTaskData(workspace);
      this.setTasks(tasks);
    } catch (err: unknown) {
      console.log("Unable to read aioptus task data.");
      console.error(err);
    }
...
async function aioptusTaskData(folder: AbsolutePath) {
